if exists (select * from sysobjects where name = 'se_rule_DNBSsamesidesymbolinxdaysunlesspricefluctuatedXpercent')
begin
	drop procedure se_rule_DNBSsamesidesymbolinxdaysunlesspricefluctuatedXpercent
	print 'PROCEDURE: se_rule_DNBSsamesidesymbolinxdaysunlesspricefluctuatedXpercent dropped'
end
go
create procedure [dbo].[se_rule_DNBSsamesidesymbolinxdaysunlesspricefluctuatedXpercent]
--exec se_rule_DNBSsamesidesymbolinxdaysunlesspricefluctuatedXpercent 10366, 1, -1, -1, 189, ACCT, 0,'DN BS  same side/symbol within x days unless the price has fluctuated to the clients favor by > X'        
   /*
" - Do not buy/sell the same security (same side/symbol) within 7 days unless the price has fluctuated to the client's favor by >5%.  
(i.e.  If the security was originally sold at $10, only sell the security again if the price is <$10.50)"
...
where 
 ...
 and se_restricted_security.rule_id = 4
...
1,
tax_lot.trade_date,
'Do not buy/sell the same security (same side/symbol) within ' + CONVERT(varchar(40),#rule_param_value.Days) + ' days unless the price has fluctuated to the clients favor by ' + CAST(#rule_param_value.PriceFlucuation AS VARCHAR(50)) + '%'
,1
,4
...
'Do not buy/sell the same security (same side/symbol) within ' + CONVERT(varchar(40),#rule_param_value.Days) + ' days unless the price has fluctuated to the clients favor by ' + CAST(#rule_param_value.PriceFlucuation AS VARCHAR(50)) + '%' as direction
...
go
if @@error = 0 print 'PROCEDURE: exec se_rule_DNBSsamesidesymbolinxdaysunlesspricefluctuatedXpercent created'
else print 'PROCEDURE: exec se_rule_DNBSsamesidesymbolinxdaysunlesspricefluctuatedXpercent error on creation'
go
