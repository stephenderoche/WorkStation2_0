if exists (select * from sysobjects where name = 'se_rule_washsale')
begin
	drop procedure se_rule_washsale
	print 'PROCEDURE: se_rule_washsale dropped'
end
go
create procedure [dbo].[se_rule_washsale]
--exec se_rule_washsale 10366, 1, -1, -1, 189, ACCT, 0,'Wash Sale Pre'        

/*wash sale
“Wash Sale – Sell”:
For proposed sell orders resulting in a loss, the rule checks that no lots for the given account and security were purchased in the last 30 days.
...
where 
 ...
 and se_restricted_security.rule_id = 5
...
and tax_lot.unit_cost_base > price.latest 
...
1,
tax_lot.trade_date,
'Wash Sale - sell orders resulting in a loss, the rule checks that no lots for the given account and security were purchased in the last ' + CONVERT(varchar(40),#rule_param_value.Days) +  'days.'
,1
,5
from tax_lot
join security on
...
'Wash Sale - No buy orders because lots for the given account and security were sold for a loss in the last 30 days.' as direction
...
where se_wash_sale_list.trade_date >= ((GetDate() - (#rule_param_value.Days +1)))
 and se_wash_sale_list.real_gain_loss < 0
) test
...
go
if @@error = 0 print 'PROCEDURE: exec se_rule_washsale created'
else print 'PROCEDURE: exec se_rule_washsale error on creation'
go
