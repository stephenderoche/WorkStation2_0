...
CREATE procedure [dbo].[se_get_parent_mutual_funds_orders_info]--se_get_parent_mutual_funds_orders_info 11575,1
(       
...
create table #t_account ( account_id numeric(10));
		  ...
		  select
				...
				case when security.user_id_15 = 1 then 'Y'
				else 'N'
				end as 'internal_funds',
				case when side_code in (0,1) then market_value 
				else 0
				end as 'subs_amount',
				case when side_code in (2,3) then market_value 
				else 0
				end as 'reds_amount'
				from
				proposed_orders
